...
ratio_ProgPow = 0.001 #4 / 9 doit avoir EIP
...
def transfo(i, yourhash, ratio):
    ...
    print(f"{i} - ", NAME1.text, ACRONYME1.text, ALGO1.text, PRICE1, EMISSION1.text, NETWORKHASH1.text,resultat,"$")
...
def tout(nb_max, yourhash):
    ...
    for i in range(1, nb_max):
        ...
        if (ALGO1==element  for element in chaine) :
            try:
                for j in range(0, 20):
                    if (ALGO1.text == chaine[j]):  # faut aussi sup si c'est egale a zero car apres div par 0
