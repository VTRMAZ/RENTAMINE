...
'''
def ergo():
    hash = float(input("Entrez votre puissance: "))
    api_key = '20fa20ea-0a1a-4ca5-b7be-fbe8114e4394'
    cmc_client = coinmarketcapapi.CoinMarketCapAPI(api_key)
    response = cmc_client.tools_priceconversion(amount = 1, symbol = 'ERG', convert = 'USD')
    price_usd = response.data[0]['quote']['USD']['price']
    print(price_usd)
'''
...
if response.status_code == 200:
    yourhash = float(input("Entrez votre puissance: "))
    ...
    print('Prix en dollars:', priceendollar)
    print(resultat)
else:
